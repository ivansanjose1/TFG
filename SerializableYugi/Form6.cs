            ...
            try
            {
                switch (focuslistbox)
                {
                    case 1:
                        ...
                        listBoxMonstruos.SelectedItem = null;
                        break;
                    case 2:
                        listBoxBaraja.Items.Add(listBoxMagia.SelectedItem);
                        listBoxMagia.SelectedItem = null;
                        break;
                    case 3:
                        listBoxBaraja.Items.Add(listBoxTrampas.SelectedItem);
                        listBoxTrampas.SelectedItem = null;
                        break;
                    case 4:
                        listBoxBaraja.Items.Add(Sugerencia.SelectedItem);
                        Sugerencia.SelectedItem = null;
                        break;
                }
            }
            catch (Exception) { MessageBox.Show("Selecciona una carta"); }
        }
        ...
        private void SugerirTrampas(Monstruo iimons)
        {
            try
            {
                ...
                while (fsTrampas.Position < fsTrampas.Length)
                {
                    ...
                    if (iimons.Nombre.ToLower().Contains(trampa.Soporte.ToLower()) || iimons.Tipo.ToLower() == trampa.Soporte.ToLower() && trampa.Soporte != "" && trampa.Soporte.ToLower() != "no")
                    {
                        Invoke(new Action(() =>
                        {
